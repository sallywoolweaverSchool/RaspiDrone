...
def run(estimation_model: str, tracker_type: str, classification_model: str,
        label_file: str, camera_id: int, width: int, height: int, drone: Drone) -> None:
  ...
  while cap.isOpened():
    ...
    print(list_persons[0].keypoints[10].coordinate.y)

    if(list_persons[0].keypoints[10].coordinate.y<100):
        #drone.set_throttle(50)
        drone.go(0,0,0,50,.25)
    elif(list_persons[0].keypoints[10].coordinate.y>350):
        #drone.set_throttle(50)
        drone.go(0,0,0,-75,.50)
    
    if(list_persons[0].keypoints[9].coordinate.y<10):
        drone.land()   
    if(list_persons[0].keypoints[9].coordinate.y <200 and list_persons[0].keypoints[10].coordinate.y >100):
        drone.flip("front")
   # print(list_persons[0].keypoints[9])
    #print(list_persons[0].keypoints[5])
    #print(list_persons[0].keypoints)
    if classifier:
      # Check if all keypoints are detected bef
      # ore running the classifier.
...
def main():
  ...
  run(args.model, args.tracker, args.classifier, args.label_file,
      int(args.cameraId), args.frameWidth, args.frameHeight, drone)
